    ...
    while(ros::ok())
    {
        ...
        else
        {
            ...
            else
            {
                if(!parking_control.parkDetected){
                ...
                }else if(parking_control.parkDetected && !in_start_position && !in_turn_position && ! in_goal_position && !in_end_position){
                ...
                }else if(in_start_position && parking_control.parkDetected && !in_turn_position && !in_goal_position && !in_end_position){
                    ...
                    if(parking_control.pos_y > parking_control.y_end + POS_TOLERANCE){
                        parking_control.control_servo.x = 1545;
                        parking_control.control_servo.y = 1500;
                        std::cout << "driving to goal position backward" << std::endl;
                    }
                    else if(parking_control.pos_y >= parking_control.y_end - POS_TOLERANCE && parking_control.pos_y <= parking_control.y_end + POS_TOLERANCE){
                        parking_control.control_servo.x = 1500;
                        parking_control.control_servo.y = 1500;
                        in_end_position = true;
                    }else{
                        std::cout << "ERROR, MISSED END POSITION" << std::endl;
                    }
                }else if(in_start_position && parking_control.parkDetected && in_turn_position && in_goal_position && in_end_position){
                    std::cout << "reached end position" << std::endl;
                }else{
